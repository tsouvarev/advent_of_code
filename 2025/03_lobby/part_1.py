...
There are batteries nearby
that can supply emergency power to the escalator for just such an occasion.
...
the joltage that the bank produces is equal
...
def find_max_joltage(raw_bank: str) -> int:
    bank = _parse_bank(raw_bank)
    first, second = 0, 0
    ...
    for battery in bank:
        if second <= battery:
            ...
            second, first = battery, second

    return int(f"{first}{second}")


def _parse_bank(raw_bank: str) -> Bank:
...
with open("2025/03_lobby/input.txt") as f:
    res = 0
    for line in f:
        res += find_max_joltage(line.strip())
    print(res)
