...
Instead, programs pass messages between each other
...
For example, suppose you go door-to-door
like a travelling salesman and record the following list:
...
def connect_pipes(raw_pipes: list[str]) -> int:
    ...
    while queue:
        ...
        for neighbor in pipes.get(node, []):
...
assert connect_pipes(raw_pipes) == 6
