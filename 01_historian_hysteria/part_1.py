...
Instead, the Elves discover an assortment of notes
and lists of historically significant locations!
...
the historically significant locations are listed not by name
...
assert list_diff([], []) == 0
assert list_diff([1], [1]) == 0
assert list_diff([1], [2]) == 1
assert list_diff([2], [1]) == 1
assert list_diff([3, 4, 2, 1, 3, 3], [4, 3, 5, 3, 9, 3]) == 11

with open("01_historian_hysteria/input.txt") as f:
    x, y = [], []
    for l in f:
        els = l.split("   ")
        x.append(int(els[0]))
