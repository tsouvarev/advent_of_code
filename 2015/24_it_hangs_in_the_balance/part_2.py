...
Of these, there are three arrangements
that put the minimum (two) number of packages in the first group: 11 4, 10 5, and 8 7.
...
def find_first_group(weights: list[int]) -> list[int]:
    ...
    for i in range(len(weights)):
        groups = [c for c in combinations(weights, i) if sum(c) == group_size]
        if groups:
            return min(reduce(mul, g) for g in groups)
    return None


assert find_first_group([1, 1, 1, 1]) == 1
assert find_first_group([1, 2, 3, 4, 5, 7, 8, 9, 10, 11]) == 44


with open("2015/24_it_hangs_in_the_balance/input.txt") as f:
    weights = [int(line.strip()) for line in f]
    print(find_first_group(weights))
