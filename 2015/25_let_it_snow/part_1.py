...
asking you to enter a code from the instruction manual.
...
start+sum(range(1, row-1))+(row-1)*(col-1)+sum(range(1, col))
(2, 1) -> 1+(1)+(1)*0+0 = 2
(1, 2) -> 1+0+0+1
...
how the codes are actually generated. The first code is 20151125.
...
start with the previous value, 20151125. Multiply it by 252533 to get 5088824049625.
...
def find_code(row: int, col: int) -> int:
    index = _convert_row_col_to_index(row, col)
    prev = 20151125
    pos = 1
...
def _convert_row_col_to_index(row: int, col: int) -> int:
    i, j, pos = 1, 1, 1
...
assert _convert_row_col_to_index(1, 1) == 1
assert _convert_row_col_to_index(2, 1) == 2
assert _convert_row_col_to_index(1, 2) == 3
assert _convert_row_col_to_index(3, 1) == 4
assert _convert_row_col_to_index(2, 2) == 5
assert _convert_row_col_to_index(1, 3) == 6

assert find_code(1, 1) == 20151125
assert find_code(2, 1) == 31916031
assert find_code(1, 2) == 18749137
assert find_code(3, 1) == 16080970
assert find_code(2, 2) == 21629792
assert find_code(1, 3) == 17289845

print(find_code(2978, 3083))
