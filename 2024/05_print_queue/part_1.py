...
the squadron of scholars suggests
...
and while The Historians continue their search of this facility,
an Elf operating a very familiar printer beckons you over.

The Elf must recognize you,
because they waste no time explaining
...
For some reason,
the Elves also need to know the middle page number of each update being printed.
...
def is_update_correct(update, rules):
    ...
    for page, page_after in rules:
        pages_after[page].add(page_after)

    seen_pages = set()
    violations = []

    for el in update:
        if el in pages_after and (violation := seen_pages & pages_after[el]):
            violations.append((el, violation))
        seen_pages.add(el)

    return violations


def get_middle_sums_of_correct_updates(updates, rules):
    res = 0
...
assert is_update_correct((), []) == []
assert is_update_correct((1,), []) == []
assert is_update_correct((1, 2), [(1, 2)]) == []
assert is_update_correct((1, 2), [(2, 1)]) == [(2, {1})]
assert is_update_correct((2, 1), [(1, 2)]) == [(1, {2})]
...
assert is_update_correct((75, 97, 47, 61, 53), rules) == [(97, {75})]
assert is_update_correct((61, 13, 29), rules) == [(29, {13})]
...
assert (
    get_middle_sums_of_correct_updates(
...
with open("2024/05_print_queue/rules.txt") as f:
    rules = [tuple(map(int, l.split("|"))) for l in f]


with open("2024/05_print_queue/updates.txt") as f:
