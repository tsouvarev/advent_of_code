...
def fix_mul(s):
    all_muls = re.findall(r"mul\((\d{1,3}),(\d{1,3})\)", s)
    return sum(mul(*map(int, args)) for args in all_muls)


assert fix_mul("mul(123,4)") == 123 * 4
assert fix_mul("mul(44,46)") == 44 * 46
assert fix_mul("mul(4,4)") == 4 * 4
assert fix_mul("mul(4,4)xmul(2,3)") == 4 * 4 + 2 * 3
assert (
    fix_mul("xmul(2,4)%&mul[3,7]!@^do_not_mul(5,5)+mul(32,64]then(mul(11,8)mul(8,5))")
    == 2 * 4 + 5 * 5 + 11 * 8 + 8 * 5
)


with open("2024/03_mull_it_over/input.txt") as f:
